...
class ApiController extends Controller
{
    ...
    public function generateQr(Request $request)
    {
        ...
        // Buscar device por api_key
        $device = Device::whereApiKey($request->api_key)->first();

        if (!$device) {
            return response()->json(['status' => false, 'msg' => 'Invalid api key!'], Response::HTTP_BAD_REQUEST);
        }

        // Verificar que el device coincida
        if ($device->body != $request->device) {
            return response()->json(['status' => false, 'msg' => 'Device does not match api key!'], Response::HTTP_BAD_REQUEST);
        }
